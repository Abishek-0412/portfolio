      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-base md:text-lg text-muted-foreground max-w-2xl mx-auto font-medium">
            Comprehensive knowledge across the full stack, built through years
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            {portfolioData.strengths.map((strength, idx) => {
              ...
              const colors =
                strengthIcons[idx]?.color || "from-gray-500 to-gray-900";
              const iconColor =
                strengthIcons[idx]?.iconColor || "text-gray-400";
              ...
              return (
                ...
                >
                  {/* Top Gradient Line */}
                  <div
                    className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${colors} opacity-30 group-hover:opacity-100 transition-opacity`}
                  />

                  {/* Icon */}
                  <div className="mb-8 mt-4 relative">
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${colors} blur-2xl opacity-10 group-hover:opacity-30 transition-opacity rounded-full`}
                    />
                    <Icon
                      size={56}
                      className={`${iconColor} drop-shadow-[0_0_15px_rgba(0,0,0,0.5)] relative z-10 transition-transform duration-500 group-hover:scale-110`}
                    />
                  </div>
